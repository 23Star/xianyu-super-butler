...
const READ_WATERMARKS_STORAGE_KEY = 'xianyu-message-read-watermarks-v1';
...
const isCoveredByReadWatermark = (
  ...
  return lastMessageTime < watermark.lastMessageTime
    || (lastMessageTime === watermark.lastMessageTime && unreadCount <= watermark.unreadCount);
};
  ...
  const loadConversations = async (silent = false) => {
    ...
    try {
      ...
      const normalized = incoming.map((conversation) => {
        const isOpen = isActive && conversation.cid === nextCid;
